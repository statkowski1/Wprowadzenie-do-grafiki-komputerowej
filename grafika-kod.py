...
class Histogram(Histogram):
    ...
    def to_cumulated(self) -> 'Histogram':
        ...
        m[0] = self.values[:, :, 0].ravel().min()
        m[1] = self.values[:, :, 1].ravel().min()
        m[2] = self.values[:, :, 2].ravel().min()
        M: np.ndarray = np.zeros(3)
        M[0] = self.values[:, :, 0].ravel().max()
        M[1] = self.values[:, :, 1].ravel().max()
        M[2] = self.values[:, :, 2].ravel().max()
        wynik:np.ndarray = np.zeros(self.values.shape)
        print(self.values.shape[2])
        for i in range(0, self.values.shape[0]):
            for j in range(0, self.values.shape[1]):
                for k in range(0, self.values.shape[2]):
                    wynik[i][j][k] = (self.values[i][j][k] - m[k]) * (255 / (M[k] - m[k]))
