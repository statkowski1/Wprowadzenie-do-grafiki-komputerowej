        ...
        for i in range(0, self.values.shape[0]):
            for j in range(0, self.values.shape[1]):
                for k in range(0, self.values.shape[2]):
                    wynik[self.values[i][j][k].astype('int64')][k] += 1
        for k in range(0, 3):
    ...
    def threshold(self, value: int) -> BaseImage:
        ...
        for i in range(0, int(self.data.shape[0]-1)):
            for j in range(0, int(self.data.shape[1]-1)):
                for k in range(0, int(self.data.shape[2]-1)):
                    if(self.data[i][j][k]>=value):
                        wynik[i][j][k] = 255
                    elif(self.data[i][j][k]<value):
                        wynik[i][j][k] = 0
        imshow(wynik)
        plt.plot()
        return wynik
